...
def precision(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        # output is shape (N, 2048, 7, 7) (ie. out of final layer of r50 backbone)
        #_, pred = torch.topk(output, max(topk), dim=1, largest=True, sorted=True)
...
def dobs_transform():
    ...
    return torchvision.transforms.Compose([resize_transform, 
...
class Validator(object):
    ...
    def __call__(self, x, y, activations=False, gradients=False):
        ...
        ######################
        # (amarvi): 
        # here is where you extract the features. change mode to appropriate 
        # openselfsup/models/....py --> add a mode in forward()
        ######################
        if self.task is None:
            # print(x.shape, type(x))
            output = self.model(img=x, mode='feature') #self.model(x=x, activations=activations, gradients=gradients)
        else:
            output = self.model(x=x, task=self.task, activations=activations, gradients=gradients)
        # prec_1, prec_5 = precision(output=output, target=y, topk=(1,5))
        # prec_1 /= len(output)
        # prec_5 /= len(output)
        # print('out', output.shape, 'y', y.shape)
        # loss = self.criterion(output,y)
        # return loss.item(), prec_1, prec_5, output
        return 'eh', 'uh', 'oh', output
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='act/grad')):
        ...
        if trainer is not None:
            ...
            with torch.no_grad():   
                ...
                # here is where amarvi gets an issue  
                # act should be shape (N, 2048, 7, 7)
                _, _, _, act = validator(x=x, y=y, activations=activations, gradients=False)          
                # print('act 1', act.shape)
            # if consol:
